<?xml version = '1.0' encoding = 'ISO-8859-1' ?>
<asm name="0">
	<cp>
		<constant value="atl"/>
		<constant value="links"/>
		<constant value="NTransientLinkSet;"/>
		<constant value="col"/>
		<constant value="J;"/>
		<constant value="root"/>
		<constant value="MUniqueLazyRule!Liste;"/>
		<constant value="main"/>
		<constant value="A"/>
		<constant value="0"/>
		<constant value="OclParametrizedType"/>
		<constant value="#native"/>
		<constant value="Collection"/>
		<constant value="J.setName(S):V"/>
		<constant value="OclSimpleType"/>
		<constant value="OclAny"/>
		<constant value="J.setElementType(J):V"/>
		<constant value="TransientLinkSet"/>
		<constant value="A.__initroot():V"/>
		<constant value="A.__matcher__():V"/>
		<constant value="A.__exec__():V"/>
		<constant value="self"/>
		<constant value="__initroot"/>
		<constant value="Sequence"/>
		<constant value="QJ.first():J"/>
		<constant value="4:44-4:56"/>
		<constant value="__matcher__"/>
		<constant value="A.__matchListe():V"/>
		<constant value="__matchListe"/>
		<constant value="Liste"/>
		<constant value="UniqueLazyRule"/>
		<constant value="IN"/>
		<constant value="MMOF!Classifier;.allInstancesFrom(S):QJ"/>
		<constant value="CJ.union(CJ):CJ"/>
		<constant value="1"/>
		<constant value="B.not():B"/>
		<constant value="37"/>
		<constant value="TransientLink"/>
		<constant value="NTransientLink;.setRule(MATL!Rule;):V"/>
		<constant value="in_liste"/>
		<constant value="NTransientLink;.addSourceElement(SJ):V"/>
		<constant value="liste"/>
		<constant value="NTransientLink;.addTargetElement(SJ):V"/>
		<constant value="NTransientLinkSet;.addLink(NTransientLink;):V"/>
		<constant value="20:11-20:31"/>
		<constant value="__resolve__"/>
		<constant value="J"/>
		<constant value="J.oclIsKindOf(J):B"/>
		<constant value="18"/>
		<constant value="NTransientLinkSet;.getLinkBySourceElement(S):QNTransientLink;"/>
		<constant value="J.oclIsUndefined():J"/>
		<constant value="15"/>
		<constant value="NTransientLink;.getTargetFromSource(J):J"/>
		<constant value="17"/>
		<constant value="30"/>
		<constant value="2"/>
		<constant value="A.__resolve__(J):J"/>
		<constant value="QJ.including(J):QJ"/>
		<constant value="QJ.flatten():QJ"/>
		<constant value="e"/>
		<constant value="value"/>
		<constant value="resolveTemp"/>
		<constant value="S"/>
		<constant value="NTransientLink;.getNamedTargetFromSource(JS):J"/>
		<constant value="name"/>
		<constant value="__exec__"/>
		<constant value="NTransientLinkSet;.getLinksByRule(S):QNTransientLink;"/>
		<constant value="A.__applyListe(NTransientLink;):V"/>
		<constant value="Object"/>
		<constant value="MUniqueLazyRule!Object;"/>
		<constant value="NTransientLinkSet;.getLinkByRuleAndSourceElement(SJ):QNTransientLink;"/>
		<constant value="11"/>
		<constant value="42"/>
		<constant value="in_object"/>
		<constant value="object"/>
		<constant value="NTransientLinkSet;.addLink2(NTransientLink;B):V"/>
		<constant value="10:3-12:4"/>
		<constant value="11:14-11:23"/>
		<constant value="11:14-11:28"/>
		<constant value="11:6-11:28"/>
		<constant value="__applyListe"/>
		<constant value="NTransientLink;"/>
		<constant value="NTransientLink;.getSourceElement(S):J"/>
		<constant value="NTransientLink;.getTargetElement(S):J"/>
		<constant value="3"/>
		<constant value="J.allInstancesFrom(J):J"/>
		<constant value="4"/>
		<constant value="J.Object(J):J"/>
		<constant value="CJ.including(J):CJ"/>
		<constant value="contents"/>
		<constant value="21:16-21:37"/>
		<constant value="21:55-21:59"/>
		<constant value="21:16-21:60"/>
		<constant value="21:74-21:84"/>
		<constant value="21:92-21:93"/>
		<constant value="21:74-21:94"/>
		<constant value="21:16-21:95"/>
		<constant value="21:4-21:95"/>
		<constant value="link"/>
	</cp>
	<field name="1" type="2"/>
	<field name="3" type="4"/>
	<field name="5" type="6"/>
	<operation name="7">
		<context type="8"/>
		<parameters>
		</parameters>
		<code>
			<load arg="9"/>
			<push arg="10"/>
			<push arg="11"/>
			<new/>
			<dup/>
			<push arg="12"/>
			<call arg="13"/>
			<dup/>
			<push arg="14"/>
			<push arg="11"/>
			<new/>
			<dup/>
			<push arg="15"/>
			<call arg="13"/>
			<call arg="16"/>
			<set arg="3"/>
			<load arg="9"/>
			<push arg="17"/>
			<push arg="11"/>
			<new/>
			<set arg="1"/>
			<load arg="9"/>
			<call arg="18"/>
			<load arg="9"/>
			<call arg="19"/>
			<load arg="9"/>
			<call arg="20"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="0" name="21" begin="0" end="26"/>
		</localvariabletable>
	</operation>
	<operation name="22">
		<context type="8"/>
		<parameters>
		</parameters>
		<code>
			<load arg="9"/>
			<push arg="23"/>
			<push arg="11"/>
			<new/>
			<call arg="24"/>
			<set arg="5"/>
		</code>
		<linenumbertable>
			<lne id="25" begin="1" end="4"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="0" name="21" begin="0" end="5"/>
		</localvariabletable>
	</operation>
	<operation name="26">
		<context type="8"/>
		<parameters>
		</parameters>
		<code>
			<load arg="9"/>
			<call arg="27"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="0" name="21" begin="0" end="1"/>
		</localvariabletable>
	</operation>
	<operation name="28">
		<context type="8"/>
		<parameters>
		</parameters>
		<code>
			<push arg="29"/>
			<push arg="30"/>
			<findme/>
			<push arg="23"/>
			<push arg="11"/>
			<new/>
			<swap/>
			<dup_x1/>
			<push arg="31"/>
			<call arg="32"/>
			<call arg="33"/>
			<swap/>
			<pop/>
			<iterate/>
			<store arg="34"/>
			<pusht/>
			<call arg="35"/>
			<if arg="36"/>
			<load arg="9"/>
			<get arg="1"/>
			<push arg="37"/>
			<push arg="11"/>
			<new/>
			<dup/>
			<push arg="29"/>
			<call arg="38"/>
			<dup/>
			<push arg="39"/>
			<load arg="34"/>
			<call arg="40"/>
			<dup/>
			<push arg="41"/>
			<push arg="29"/>
			<push arg="30"/>
			<new/>
			<call arg="42"/>
			<call arg="43"/>
			<enditerate/>
		</code>
		<linenumbertable>
			<lne id="44" begin="32" end="34"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="39" begin="14" end="36"/>
			<lve slot="0" name="21" begin="0" end="37"/>
		</localvariabletable>
	</operation>
	<operation name="45">
		<context type="8"/>
		<parameters>
			<parameter name="34" type="46"/>
		</parameters>
		<code>
			<load arg="34"/>
			<load arg="9"/>
			<get arg="3"/>
			<call arg="47"/>
			<if arg="48"/>
			<load arg="9"/>
			<get arg="1"/>
			<load arg="34"/>
			<call arg="49"/>
			<dup/>
			<call arg="50"/>
			<if arg="51"/>
			<load arg="34"/>
			<call arg="52"/>
			<goto arg="53"/>
			<pop/>
			<load arg="34"/>
			<goto arg="54"/>
			<push arg="23"/>
			<push arg="11"/>
			<new/>
			<load arg="34"/>
			<iterate/>
			<store arg="55"/>
			<load arg="9"/>
			<load arg="55"/>
			<call arg="56"/>
			<call arg="57"/>
			<enditerate/>
			<call arg="58"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="2" name="59" begin="23" end="27"/>
			<lve slot="0" name="21" begin="0" end="29"/>
			<lve slot="1" name="60" begin="0" end="29"/>
		</localvariabletable>
	</operation>
	<operation name="61">
		<context type="8"/>
		<parameters>
			<parameter name="34" type="46"/>
			<parameter name="55" type="62"/>
		</parameters>
		<code>
			<load arg="9"/>
			<get arg="1"/>
			<load arg="34"/>
			<call arg="49"/>
			<load arg="34"/>
			<load arg="55"/>
			<call arg="63"/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="0" name="21" begin="0" end="6"/>
			<lve slot="1" name="60" begin="0" end="6"/>
			<lve slot="2" name="64" begin="0" end="6"/>
		</localvariabletable>
	</operation>
	<operation name="65">
		<context type="8"/>
		<parameters>
		</parameters>
		<code>
			<load arg="9"/>
			<get arg="1"/>
			<push arg="29"/>
			<call arg="66"/>
			<iterate/>
			<store arg="34"/>
			<load arg="9"/>
			<load arg="34"/>
			<call arg="67"/>
			<enditerate/>
		</code>
		<linenumbertable>
		</linenumbertable>
		<localvariabletable>
			<lve slot="1" name="59" begin="5" end="8"/>
			<lve slot="0" name="21" begin="0" end="9"/>
		</localvariabletable>
	</operation>
	<operation name="68">
		<context type="8"/>
		<parameters>
			<parameter name="34" type="69"/>
		</parameters>
		<code>
			<load arg="9"/>
			<get arg="1"/>
			<push arg="68"/>
			<load arg="34"/>
			<call arg="70"/>
			<dup/>
			<call arg="50"/>
			<if arg="71"/>
			<load arg="34"/>
			<call arg="52"/>
			<goto arg="72"/>
			<load arg="9"/>
			<get arg="1"/>
			<push arg="37"/>
			<push arg="11"/>
			<new/>
			<dup/>
			<push arg="68"/>
			<call arg="38"/>
			<dup/>
			<push arg="73"/>
			<load arg="34"/>
			<call arg="40"/>
			<dup/>
			<push arg="74"/>
			<push arg="68"/>
			<push arg="30"/>
			<new/>
			<dup/>
			<store arg="55"/>
			<call arg="42"/>
			<pushf/>
			<call arg="75"/>
			<load arg="55"/>
			<dup/>
			<load arg="9"/>
			<load arg="34"/>
			<get arg="64"/>
			<call arg="56"/>
			<set arg="64"/>
			<pop/>
			<load arg="55"/>
		</code>
		<linenumbertable>
			<lne id="76" begin="23" end="30"/>
			<lne id="77" begin="36" end="36"/>
			<lne id="78" begin="36" end="37"/>
			<lne id="79" begin="34" end="39"/>
			<lne id="76" begin="33" end="40"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="2" name="74" begin="29" end="41"/>
			<lve slot="0" name="21" begin="0" end="41"/>
			<lve slot="1" name="73" begin="0" end="41"/>
		</localvariabletable>
	</operation>
	<operation name="80">
		<context type="8"/>
		<parameters>
			<parameter name="34" type="81"/>
		</parameters>
		<code>
			<load arg="34"/>
			<push arg="39"/>
			<call arg="82"/>
			<store arg="55"/>
			<load arg="34"/>
			<push arg="41"/>
			<call arg="83"/>
			<store arg="84"/>
			<load arg="84"/>
			<dup/>
			<load arg="9"/>
			<push arg="23"/>
			<push arg="11"/>
			<new/>
			<push arg="68"/>
			<push arg="30"/>
			<findme/>
			<push arg="31"/>
			<call arg="85"/>
			<iterate/>
			<store arg="86"/>
			<getasm/>
			<load arg="86"/>
			<call arg="87"/>
			<call arg="88"/>
			<enditerate/>
			<call arg="56"/>
			<set arg="89"/>
			<pop/>
		</code>
		<linenumbertable>
			<lne id="90" begin="14" end="16"/>
			<lne id="91" begin="17" end="17"/>
			<lne id="92" begin="14" end="18"/>
			<lne id="93" begin="21" end="21"/>
			<lne id="94" begin="22" end="22"/>
			<lne id="95" begin="21" end="23"/>
			<lne id="96" begin="11" end="25"/>
			<lne id="97" begin="9" end="27"/>
		</linenumbertable>
		<localvariabletable>
			<lve slot="4" name="59" begin="20" end="24"/>
			<lve slot="2" name="39" begin="3" end="28"/>
			<lve slot="3" name="41" begin="7" end="28"/>
			<lve slot="0" name="21" begin="0" end="28"/>
			<lve slot="1" name="98" begin="0" end="28"/>
		</localvariabletable>
	</operation>
</asm>
