...
public class AtlStackFrame extends AtlDebugElement implements IStackFrame {
	...
	public AtlStackFrame(AtlThread thread, ObjectReference stackFrame, AtlNbCharFile atlnbcharfile,
		...
		Value slv = stackFrame.call("getSourceLocation", Collections.<Value>emptyList()); //$NON-NLS-1$
	...
	public IVariable[] getVariables() throws DebugException {
		...
		ObjectReference lvs = (ObjectReference)stackFrame.call("getLocalVariables", Collections.<Value>emptyList()); //$NON-NLS-1$
		// Set of local variable names
		ObjectReference lvns = (ObjectReference)lvs.call("getKeys", Collections.<Value>emptyList()); //$NON-NLS-1$
		// Sequence of local variable names
		lvns = (ObjectReference)lvns.call("asSequence", Collections.<Value>emptyList()); //$NON-NLS-1$
		// number of localVariables
		IntegerValue nblvs = (IntegerValue)lvns.call("size", Collections.<Value>emptyList()); //$NON-NLS-1$
		List<AtlVariable> atlVars = new ArrayList<AtlVariable>();
		for (int i = 1; i <= nblvs.getValue(); i++) {
			...
			Value value = lvs.call("get", Arrays.asList(new Value[] {name})); //$NON-NLS-1$
			...
			if (name.getValue().matches("^[0-9]*$")) { //$NON-NLS-1$
				Value val = stackFrame
						.call(
								"resolveVariableName", Arrays.asList(new Value[] {IntegerValue.valueOf(Integer.parseInt(name.getValue()))})); //$NON-NLS-1$
			...
			AtlVariable atlVariable = vars.get(sname);
			if (atlVariable == null) {
	...
	public IStorage getDisassembled() {
		...
		List<Value> l = debugger.requestMessage(ADWPDebugger.CMD_DISASSEMBLE,
				Arrays.asList(new Value[] {stackFrame})).getArgs();
		for (Iterator<Value> i = l.iterator(); i.hasNext();) {
