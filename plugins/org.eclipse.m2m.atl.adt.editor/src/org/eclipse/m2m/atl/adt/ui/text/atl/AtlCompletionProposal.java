...
public class AtlCompletionProposal implements ICompletionProposalExtension, ICompletionProposalExtension2, ICompletionProposalExtension3, ICompletionProposal, Comparable {
	...
	private static Color getBackgroundColor(StyledText text) {
		...
		RGB rgb = PreferenceConverter.getColor(preference,
				AtlPreferenceConstants.CODEASSIST_REPLACEMENT_BACKGROUND);
	...
	private static Color getForegroundColor(StyledText text) {
		...
		IPreferenceStore preference = AtlUIPlugin.getDefault().getPreferenceStore();
		RGB rgb = PreferenceConverter.getColor(preference,
				AtlPreferenceConstants.CODEASSIST_REPLACEMENT_FOREGROUND);
		AtlTextTools textTools = AtlUIPlugin.getDefault().getTextTools();
		return textTools.getColorManager().getColor(rgb);
	}
	...
	public void apply(IDocument document, char trigger, int offset) {
		try {
			...
			boolean isSmartTrigger = trigger == ';'
			...
			if (isSmartTrigger) {
				...
				/**
				 * TODO create smart semi colon auto edit strategy SmartSemicolonAutoEditStrategy strategy=
				 * new SmartSemicolonAutoEditStrategy(IAtlPartitions.Atl_PARTITIONING);
			...
			if (fTextViewer != null && string != null) {
				...
				if (index != -1 && index + 1 == fCursorPosition) {
					...
					if (preferenceStore.getBoolean(AtlPreferenceConstants.TYPING_CLOSE_BRACKETS)) {
						...
						group.addPosition(new LinkedPosition(document, newOffset, 0,
	...
	public boolean validate(IDocument document, int offset, DocumentEvent event) {
		...
		/*
		 * See http://dev.eclipse.org/bugs/show_bug.cgi?id=17667 String word= fReplacementString;
		 */
