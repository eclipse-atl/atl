...
public abstract class ModifyDialogTabPage implements IModifyDialogTabPage {
	...
	protected Button fShowInvisibleButton;

	/**
	 * This is the default listener for any of the Preference classes. It is added by the respective factory
	 * methods and updates the page's preview on each change.
	 */
	...
	protected final class ComboPreference extends Preference {
		...
		public ComboPreference(Composite composite, int numColumns, Map<String, String> preferences, String key,
			...
			super(preferences, key);
			/*
			 * if (values == null || items == null || text == null) throw new IllegalArgumentException(
			...
			fCombo.setLayoutData(createGridData(1, GridData.HORIZONTAL_ALIGN_FILL, fCombo.computeSize(
	...
	protected final class NumberPreference extends Preference {
		...
		private IStatus createErrorStatus() {
			/*return new Status(IStatus.ERROR, JavaPlugin.getPluginId(), 0, Messages.format(
			...
			if (hasKey) {
				...
				try {
					...
					/*final String message = Messages.format(
							...
							getKey());
					JavaPlugin
							.log(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.OK, message, e));*/
					s = ""; //$NON-NLS-1$
				}
	...
	protected final class StringPreference extends Preference {
		 ...
		 *            the composite on which the SWT widgets are added.
		 * @param numColumns
		 *            the number of columns in the composite's {@link GridLayout}
		 * @param preferences
		 *            the map to store the values.
		 * @param key
		 *            the key to store the values.
		...
		public StringPreference(Composite composite, int numColumns, Map<String, String> preferences, String key,
			...
			final int length = 30;
			fText.setLayoutData(createGridData(1, GridData.HORIZONTAL_ALIGN_BEGINNING, fPixelConverter
					.convertWidthInCharsToPixels(length)));
	 ...
	 * sessions. Each ModifyDialogTabPage has its own instance, and it should add all relevant controls upon
	 * creation, always in the same sequence. This established a mapping of controls to indexes, which allows
	 * to restore the focus in a later session. The index is saved in the dialog settings, and there is only
	 * one common preference for all tab pages. It is always the currently active tab page which stores its
	 * focus index.
	 */
	...
	/**
	 * Layout used for the settings part. Makes sure to show scrollbars if necessary. The settings part needs
	 * to be layouted on resize.
	 */
	...
	/**
	 * The default focus manager. This widget knows all widgets which can have the focus and listens for
	 * focusGained events, on which it stores the index of the current focus holder. When the dialog is
	 * restarted, <code>restoreFocus()</code> sets the focus to the last control which had it. The standard
	 * Preference object are managed by this focus manager if they are created using the respective factory
	 * methods. Other SWT widgets can be added in subclasses when they are created.
	 */
	 ...
	 * <p>
	 * Subclasses should implement <code>doCreatePreferences</code> and <code>doCreatePreview</code> may also
	...
	/**
	 * This method is called after all controls have been allocated, including the preview. It can be used to
	 * set the preview text and to create listeners.
	 */
	...
	/**
	 * Create the right side of the modify dialog. By default, the preview is displayed there. Subclasses can
	 * override this method in order to customize the right-hand side of the dialog.
	 * 
	 * @param composite
	 *            Composite to create in
	 * @param numColumns
	...
	protected Composite doCreatePreviewPane(Composite composite, int numColumns) {
		...
		createLabel(numColumns, composite, "Preview:"); //$NON-NLS-1$

		final AtlPreview preview = doCreateAtlPreview(composite);
	...
	/**
	 * To be implemented by subclasses. This method should return an instance of JavaPreview. Currently, the
	 * choice is between CompilationUnitPreview which contains a valid compilation unit, or a SnippetPreview
	 * which formats several independent code snippets and displays them in the same window.
	 * 
	 * @param parent
	 *            Parent composite
	 * @return Created preview
	 */
	protected abstract AtlPreview doCreateAtlPreview(Composite parent);
	...
	/**
	 * Set the status field on the dialog. This can be used by tab pages to report inconsistent input. The OK
	 * button is disabled if the kind is IStatus.ERROR.
	 * 
	 * @param status
	...
	/*
	 * Create a GridLayout with the default margin and spacing settings, as well as the specified number of
	 * columns.
	 */
	...
	protected static Label createLabel(int numColumns, Composite parent, String text, int gridDataStyle) {
		...
		label.setLayoutData(createGridData(numColumns, gridDataStyle, pixelConverter
	...
	/*
	 * Convenience method to create a NumberPreference. The widget is registered as a potential focus holder,
	 * and the default updater is added.
	 */
	...
	/*
	 * Convenience method to create a StringPreference. The widget is registered as a potential focus holder,
	 * and the default updater is added.
	 * @since 3.6
	 */
	...
	/*
	 * Convenience method to create a ComboPreference. The widget is registered as a potential focus holder,
	 * and the default updater is added.
	 */
	protected ComboPreference createComboPref(Composite composite, int numColumns, String name, String key,
		...
		final ComboPreference pref = new ComboPreference(composite, numColumns, fWorkingValues, key, values,
				name, items);
		fDefaultFocusManager.add(pref);
		pref.addObserver(fUpdater);
		return pref;
	...
	/*
	 * Convenience method to create a CheckboxPreference. The widget is registered as a potential focus
	 * holder, and the default updater is added.
	 */
	protected CheckboxPreference createCheckboxPref(Composite composite, int numColumns, String name,
			String key, String[] values) {
		final CheckboxPreference pref = new CheckboxPreference(composite, numColumns, fWorkingValues, key,
				values, name);
		fDefaultFocusManager.add(pref);
		pref.addObserver(fUpdater);
		return pref;
	...
	protected RadioPreference createRadioPref(Composite composite, int numColumns, String name, String key,
			String[] values) {
		final RadioPreference pref = new RadioPreference(composite, numColumns, fWorkingValues, key, values,
				name);
		fDefaultFocusManager.add(pref);
		pref.addObserver(fUpdater);
		return pref;
	...
	public Button getShowInvisibleButton() {
		return fShowInvisibleButton;
	}
