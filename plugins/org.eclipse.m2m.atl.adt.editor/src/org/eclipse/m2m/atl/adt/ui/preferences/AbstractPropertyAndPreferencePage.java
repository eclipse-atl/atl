...
public abstract class AbstractPropertyAndPreferencePage extends PropertyPage implements IWorkbenchPropertyPage, IWorkbenchPreferencePage {
	...
	protected Map<TableItem, String> tableProperties = new HashMap<TableItem, String>();
	...
	protected void initializeFieldsToDefaultValues() {
		...
		if (tableProperties != null) {
			Iterator<TableItem> i = tableProperties.keySet().iterator();
			while (i.hasNext()) {
				TableItem ti = i.next();
				String key = tableProperties.get(ti);
				ti.setText(1, fPreferenceeStore.getDefaultString(key));
				fPreferenceeStore.setValue(tableProperties.get(ti), ti.getText(1));
			}
		}
	}
	...
	protected void initializeFieldsToUserValues() {
		...
		if (textFields != null) {
			...
			while (i.hasNext()) {
				Text t = i.next();
				String key = textFields.get(t);
				String text = fPreferenceeStore.getString(key);
	...
	protected void performDefaults() {
		...
		preview.setWorkingValues(AtlCodeFormatterPreferences.getDefaultOptions());
