...
public class AtlContentAssistPreference {
	 ...
	 * 
	 * @param assistant
	 *            the given content assistant
	 * @param store
	 *            the given preference store
	 * @param event
	...
	public static void changeConfiguration(ContentAssistant assistant, IPreferenceStore store,
		...
		if (AtlPreferenceConstants.CODEASSIST_AUTOACTIVATION_ENABLE.equals(property)) {
			assistant.enableAutoActivation(store
					.getBoolean(AtlPreferenceConstants.CODEASSIST_AUTOACTIVATION_ENABLE));
		} else if (AtlPreferenceConstants.CODEASSIST_AUTOACTIVATION_DELAY.equals(property)) {
			assistant.setAutoActivationDelay(store
					.getInt(AtlPreferenceConstants.CODEASSIST_AUTOACTIVATION_DELAY));
			...
			assistant.setProposalSelectorBackground(getColor(store,
					AtlPreferenceConstants.CODEASSIST_PROPOSALS_BACKGROUND, colorManager));
		} else if (AtlPreferenceConstants.CODEASSIST_PROPOSALS_FOREGROUND.equals(property)) {
			assistant.setProposalSelectorForeground(getColor(store,
					AtlPreferenceConstants.CODEASSIST_PROPOSALS_FOREGROUND, colorManager));
			...
			assistant.enablePrefixCompletion(store
					.getBoolean(AtlPreferenceConstants.CODEASSIST_PREFIX_COMPLETION));
		}
	...
	public static void configure(ContentAssistant assistant, IPreferenceStore store) {
		...
		assistant.enableAutoActivation(store
				.getBoolean(AtlPreferenceConstants.CODEASSIST_AUTOACTIVATION_ENABLE));
		assistant
				.setAutoActivationDelay(store.getInt(AtlPreferenceConstants.CODEASSIST_AUTOACTIVATION_DELAY));
		assistant.enableAutoInsert(store.getBoolean(AtlPreferenceConstants.CODEASSIST_AUTOINSERT));
		assistant.setProposalSelectorBackground(getColor(store,
				AtlPreferenceConstants.CODEASSIST_PROPOSALS_BACKGROUND, manager));
		assistant.setProposalSelectorForeground(getColor(store,
				AtlPreferenceConstants.CODEASSIST_PROPOSALS_FOREGROUND, manager));
		assistant.enablePrefixCompletion(store
				.getBoolean(AtlPreferenceConstants.CODEASSIST_PREFIX_COMPLETION));
	...
	private static void configureProcessor(ContentAssistant assistant, IPreferenceStore store) {
		...
		cp.restrictProposalsToVisibility(store
				.getBoolean(AtlPreferenceConstants.CODEASSIST_SHOW_VISIBLE_PROPOSALS));
		cp.restrictProposalsToMatchingCases(store
				.getBoolean(AtlPreferenceConstants.CODEASSIST_CASE_SENSITIVITY));
