 ...
 *
 * $Id: EMFReferenceModel.java,v 1.3.2.1 2008/09/11 15:15:37 dwagelaar Exp $
 */
...
public class EMFReferenceModel extends EMFModel implements ReferenceModel {
	 ...
	 * @throws IOException
	 */
	public EMFReferenceModel(ReferenceModel referenceModel, URL url, ResourceSet resourceSet) throws IOException {
		super(referenceModel, url, resourceSet);
	 ...
	 * @throws IOException
	 */
	public EMFReferenceModel(ReferenceModel referenceModel, URI uri, ResourceSet resourceSet) throws IOException {
		super(referenceModel, uri, resourceSet, false);
		registerPackages(resourceSet);
		adapt();
		addAllReferencedResources(this.getResource());
	}
	...
	public EMFReferenceModel(ReferenceModel referenceModel, Resource resource) {
		...
		registerPackages(resource.getResourceSet());
		addAllReferencedResources(this.getResource());
	}
	 ...
	 */
	public EMFReferenceModel(ReferenceModel referenceModel, String uri, ResourceSet resourceSet) {
		super(referenceModel, resourceSet.getPackageRegistry().getEPackage(uri).eResource());
		registerPackages(resourceSet);
		addAllReferencedResources(this.getResource());
	}
	...
	private void registerPackages(ResourceSet resourceSet) {
		for (Iterator i = getElementsByType(EcorePackage.eINSTANCE.getEPackage()).iterator(); i.hasNext();) {
			...
			if (resourceSet == null) continue; // generated EMF meta-models don't use a ResourceSet
