...
public class AtlSuperimposeModule {
        ...
        for (int i = 0; i < preEnd; i++) {
            ...
            if (!ins1.equals(ins2)) {
                throw new AtlSuperimposeModuleException(
                        EMFVMMessages.getString("AtlSuperimposeModule.0") +  //$NON-NLS-1$
    ...
    private void mainSanityCheck(ASMOperation main) throws AtlSuperimposeModuleException {
        ...
        if (instructions.size() < 21) {
            throw new AtlSuperimposeModuleException(
                    EMFVMMessages.getString("AtlSuperimposeModule.1") + //$NON-NLS-1$
                    String.valueOf(instructions.size()) + 
                    EMFVMMessages.getString("AtlSuperimposeModule.2")); //$NON-NLS-1$
        }
        final String instr16 = instructions.get(15).toString();
        if (!instr16.equals("set col")) { //$NON-NLS-1$
            throw new AtlSuperimposeModuleException(
                    EMFVMMessages.getString("AtlSuperimposeModule.3") + //$NON-NLS-1$
        ...
        if (indexOfInstruction(instructions, "set links", 16) == -1) { //$NON-NLS-1$
            throw new AtlSuperimposeModuleException(
                    EMFVMMessages.getString("AtlSuperimposeModule.5")); //$NON-NLS-1$
        }
    	...
    	for (Iterator i = instructions.iterator(); i.hasNext();) {
    		...
    		if (instruction instanceof Bytecode) {
    			...
    			if ((opcode == Bytecode.IF) || (opcode == Bytecode.GOTO)) {
    				int offset = Integer.parseInt((String) instr.operand);
    				if (offset >= start) {
    					offset += transpose;
    					instr.operand = String.valueOf(offset);
    				}
    			}
    		}
    	}
    ...
    private void sanityCrossCheck(ASMOperation op1, ASMOperation op2, int patternLength) throws AtlSuperimposeModuleException {
        ...
        for (int i = 0; i < limit; i++) {
            int i1 = ((Bytecode)instr1.get(i)).opcode;
            int i2 = ((Bytecode)instr2.get(i)).opcode;
            if (i1 != i2) {
                throw new AtlSuperimposeModuleException(
                        EMFVMMessages.getString("AtlSuperimposeModule.6") +  //$NON-NLS-1$
    ...
    private void sanityCheck(ASMOperation op, int patternLength) throws AtlSuperimposeModuleException {
        ...
        for (int i = 0; i < instr.size()-patternLength; i++) {
            int i1 = ((Bytecode)instr.get(i)).opcode;
            int i2 = ((Bytecode)instr.get(i+patternLength)).opcode;
            if (i1 != i2) {
