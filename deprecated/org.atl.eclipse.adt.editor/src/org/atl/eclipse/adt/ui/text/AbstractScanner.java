...
public abstract class AbstractScanner extends RuleBasedScanner {
	 ...
	 * This needs to create firstable the tokens style, and afterwards associate
	 * them to their token value.
	 */
	...
	private void adaptToColorChange(Token token, PropertyChangeEvent event) {
		...
		if(rgb != null) {
			...
			if(data instanceof TextAttribute) {
				...
				token.setData(new TextAttribute(getTextTools().getColorManager().getColor(property), oldAttr.getBackground(), oldAttr.getStyle()));
			}
		}
	}
	 ...
	 * @param boldKey the bold style associated to the key
	 * @param italicKey the italic style associated to the key
	 */
	private void addToken(String color, String boldKey, String italicKey) {
		getTextTools().getColorManager().add(
				color,
		...
		tokenMap.put(color, new Token(new TextAttribute(
				getTextTools().getColorManager().getColor(color),
				null,
				style)));
	}
	...
	private String[] getPropertyNamesBold() {
		if(propertyNamesBold == null) {
			...
			for(int i = 0; i < length; ++i) {
				propertyNamesBold[i] = getPropertyNames()[i] + AtlPreferenceConstants.__BOLD_SUFFIX; 
			}
		}
	...
	private String[] getPropertyNamesColor() {
		if(propertyNamesColor == null) {
			...
			for(int i = 0; i < length; ++i) {
				propertyNamesColor[i] = getPropertyNames()[i] + AtlPreferenceConstants.__COLOR_SUFFIX; 
			}
		}
	...
	private String[] getPropertyNamesItalic() {
		if(propertyNamesItalic == null) {
			...
			for(int i = 0; i < length; ++i) {
				propertyNamesItalic[i] = getPropertyNames()[i] + AtlPreferenceConstants.__ITALIC_SUFFIX; 
			}
		}
	 ...
	 * 
	 * @param color the color name of the key
	...
	private int indexOf(String property) {
		if(property != null) {
			...
			for(int i = 0; i < length; i++) {
				if(
						property.equals(getPropertyNamesColor()[i]) ||
						property.equals(getPropertyNamesBold()[i]) ||
	...
	public final void initialize() {
		for(int i = 0; i < getPropertyNames().length; ++i) {
			addToken(
					getPropertyNamesColor()[i],
					getPropertyNamesBold()[i],
					getPropertyNamesItalic()[i]);
		}
